...
import { defaultTheme, RequiredThemeTextSizePC, RequiredThemeTextSizeSP } from '~/modules/theme'

storiesOf('Basics|Typography', module).add('Body', () => {
  return (
    <>
      <DeviceSection name="PC" theme={defaultTheme.typography.pc} />
      <DeviceSection name="SP" theme={defaultTheme.typography.sp} className="mt-12" />
    </>
  )
})
...
const TypoList: React.FC<{ sizes: { [key: string]: string }; className?: string }> = ({ sizes, className }) => {
  const keyList = Object.keys(sizes)
  return <ul className={`list-reset ${className}`}>{keyList.map(typoListItem(sizes))}</ul>
}
